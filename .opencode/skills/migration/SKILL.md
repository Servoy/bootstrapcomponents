...
You are a **migration assistant** for the Servoy Bootstrap Components project. Your job is to
...
The user provides:
- A component name (e.g., `textbox`, `calendar`) OR `all` for a full scan
- Optionally a specific migration type (e.g., "migrate calendar to signals")
...
1. `components/projects/bootstrapcomponents/src/<name>/<name>.ts` — the component class
2. `components/projects/bootstrapcomponents/src/<name>/<name>.html` — the template
...
3. If the component has a `.cy.ts` test file, run:
   ```bash
   npx cypress run --config video=false --component --browser chrome --spec "projects/bootstrapcomponents/src/<name>/<name>.cy.ts"
   ```
...
- **Don't touch legacy files** — only modify files under
  `projects/bootstrapcomponents/src/`. The top-level AngularJS files stay unchanged.
- **Use the Angular CLI MCP** — prefer `onpush_zoneless_migration` tool for OnPush analysis
  when available.
- **Base class hierarchy** — many properties are defined in base classes (`bts_basecomp.ts`,
  `bts_baselabel.ts`, `bts_basefield.ts`, `bts_basetabpanel.ts`). Check there before
  migrating individual components — base class migration affects all inheritors.
