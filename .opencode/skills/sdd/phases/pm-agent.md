...
- Understand the dual-layer architecture:
  - Servoy spec file (`components/<name>/<name>.spec`) — defines model, handlers, API
  - Angular implementation (`components/projects/bootstrapcomponents/src/<name>/`)
- Identify patterns in existing components that should be followed
- Check `@servoy/public` base classes for inherited behaviour
- Check the base class hierarchy (`bts_basecomp.ts`, `bts_baselabel.ts`,
  `bts_basefield.ts`, `bts_basetabpanel.ts`) for shared properties
...
Example: `docs/SVY-21080-calendar-inline-date-selection.spec.md`
...
```
docs/SVY-21080-calendar-inline-date-selection.spec.md
```
