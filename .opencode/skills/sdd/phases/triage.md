...
- The Servoy component spec file (`components/<name>/<name>.spec`) — defines the
  component contract (model properties, handlers, API methods, types)
- The Angular implementation (`components/projects/bootstrapcomponents/src/<name>/`)
- The `@servoy/public` base classes and utilities
- The base class hierarchy (`bts_basecomp.ts`, `bts_baselabel.ts`, `bts_basefield.ts`,
  `bts_basetabpanel.ts`) — shared behavior across components
...
```powershell
cd "C:\Users\jcomp\git\bootstrapcomponents" ; git blame -L <start>,<end> "<file-path>"
...
- **Is the problem even in Servoy component code?** Or is it user-side (misconfiguration,
  ...
  (`@servoy/public`, `@ng-bootstrap`, `@eonasdan/tempus-dominus`, etc.)?
