...
You are a **sync verification agent** for the Servoy Bootstrap Components project. Your job is to
...
- A specific component name (e.g., `textbox`, `calendar`)
...
directories under `components/projects/bootstrapcomponents/src/` that also have a
matching directory under `components/` with a `.spec` file.

Known components: accordion, button, calendar, calendarinline, checkbox, choicegroup,
combobox, datalabel, floatlabelcalendar, floatlabelcombobox, floatlabeltextarea,
floatlabeltextbox, floatlabeltypeahead, imagemedia, label, list, select, tablesspanel,
tabpanel, textarea, textbox, typeahead

### Step 2 — For each component, read both layers

1. **Spec file:** `components/<name>/<name>.spec` — parse the JSON
2. **Angular file:** `components/projects/bootstrapcomponents/src/<name>/<name>.ts`
...
- Properties inherited from base classes (`ServoyBootstrapBaseComponent`,
...
- Some Angular components have extra internal signals/properties that are NOT in the spec
  (e.g., `_dataProviderID`, `_editable`, `_toolTipText` via `linkedSignal()`). These are
  implementation details and should NOT be flagged.
...
- Base class hierarchy provides many shared properties — check `bts_basecomp.ts`,
  `bts_baselabel.ts`, `bts_basefield.ts`, and `bts_basetabpanel.ts` for inherited inputs.
